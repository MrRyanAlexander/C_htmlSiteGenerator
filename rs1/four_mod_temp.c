...
void genFourTemp(char title[100], char description[100], char keywords[200], char url[100], char country[10], char keyword[40]) {
    
    //if (stat(url, &st) == -1) {			//check if directory already exists
        
        mkdir(url, 0700); 				//make a new directory using the new name
        strcat(url, "/index.php");		//concat the new directory name with the prefix
        FILE *site = fopen(url, "w");	//create a new file in the new directory
        //begin writing data to the new file
        ...
        fprintf(site,
                ...
                "  ga('send', 'pageview');\n"
                "</script>\n"
                "</html>", keyword, country, title, description, keywords, title);
        
        fclose(site);
    //}else{
      //  printf("Skipping duplicate URL");
        //break;
    //}
}